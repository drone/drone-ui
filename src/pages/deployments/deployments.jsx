...
export default function Deployments({ repo }) {
  ...
  let content = <p className="card">Your build list is empty</p>;
  ...
  if (isLoading) {
    ...
    content = (
      <ZeroState title="Your Deployment List is Empty." message="This list will be populated when you promote or rollback a build." />
    );
  }
  ...
  return (
    <>
      <div className={cx('deployment-list-item')}>
        ...
        <div>{build.deploy_to}</div>
        <div>{build.after.slice(0, 8)}</div>
        <div>
          <img src={build.author_avatar} />
          <span>{build.author_login}</span>
        </div>
        <div><span>{build.message}</span></div>
      </div>
    </>
  );
}
