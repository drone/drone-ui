...
export const fetchCron = async ({commit}, state) => {
	...
	const req = await fetch(`${instance}/api/repos/${namespace}/${name}/cron/${cron}`, {headers, credentials: 'same-origin'});
	const res = await req.json();
...
export const updateCron = async (dispatch, state, input) => {
	...
	const req = await fetch(`${instance}/api/repos/${namespace}/${name}/cron/${input.name}`, {headers, method: 'PATCH', body, credentials: 'same-origin'});
    const res = await req.json();
