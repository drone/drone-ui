...
export const updateNode = async ({commit}, input) => {
	...
	const req = await fetch(`${instance}/api/nodes/${input.name}`, {headers, method: 'PATCH', body, credentials: 'same-origin'});
    const res = await req.json();
...
export const deleteNode = async ({commit}, node) => {
	...
	const req = await fetch(`${instance}/api/nodes/${node.name}`, {headers, method: 'DELETE', credentials: 'same-origin'});
    const res = await req.json();
