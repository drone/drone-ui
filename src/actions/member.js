...
export const fetchMembers = async (dispatch, state) => {
	...
	const req = await fetch(`${instance}/api/repos/${namespace}/${name}/collaborators`, {headers, credentials: 'same-origin'});
...
export const deleteMember = async (dispatch, state, username) => {
	...
	const {namespace, name} = state.route.params;
	const req = await fetch(`${instance}/api/repos/${namespace}/${name}/collaborators/${username}`, {headers, method: 'DELETE', credentials: 'same-origin'});
